...
func TestRRule(t *testing.T) {
	...
	instances = r.Between(from, to, true)
...
func TestRRuleIterate(t *testing.T) {
	...
	if value, want := r.All()[0], time.Date(1997, 12, 29, 0, 0, 0, 0, time.UTC); value != want {
	...
	if value, want := r.All()[3], time.Date(1997, 9, 2, 1, 2, 0, 0, time.UTC); value != want {
		t.Errorf("r.All()[1] = %v, want %v", value, want)
	}
